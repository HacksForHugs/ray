...
class DataFrame(object):
    ...
    def __setitem__(self, key, value):
        ...
        def value_helper(i):
            # given partition number, return slice of value array corresponding to this partition
            start = sum(self._lengths[:i])
            end = start + self.lengths[i]
            return value[start:end]

        def set_array(df, key, i=False):
        ...
        if isinstance(key, (pd.Series, pd.DataFrame, list, np.ndarray)):
            raise NotImplementedError("Not yet implemented.")
        elif isinstance(value, (list, np.ndarray, pd.Series, pd.DataFrame)):
            new_df = [_deploy_func.remote(
                lambda df: set_array(df, key, i), part)
